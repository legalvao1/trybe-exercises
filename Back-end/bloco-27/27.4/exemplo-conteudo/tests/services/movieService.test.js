...
describe('Busca o filme no BD pelo Id', () => {
  const EXAMPLE_WRONG_ID = '613b466f4e877305bcb5c674';
  describe('quando não encontra o filme pelo Id', () => {
    before(() => {
      sinon.stub(MoviesModel, 'getMovieById')
        .resolves(null);
    });
    ...
    it('retorna null', async () => {
      const response = await MoviesService.getMovieById(EXAMPLE_WRONG_ID);
  ...
  describe('quando encontra o filme pelo Id', () => {
    const EXAMPLE_ID = '613b466f4e877305bcb5c673';

    before(() => {
      sinon.stub(MoviesModel, 'getMovieById')
        .resolves({
          _id: '613b466f4e877305bcb5c673',
          title: 'Example Movie',
          directedBy: 'Jane Dow',
           releaseYear: 1999,
        });
    ...
    it('retorna um objeto', async () => {
      const response = await MoviesService.getMovieById(EXAMPLE_ID);

      expect(response).to.be.an('object');
    });

    it('O item possui as propriedades: "_id", "title", "releaseYear" e "directedBy"', async () => {
      const item = await MoviesService.getMovieById(EXAMPLE_ID);
